...
(function(){
      ...
      cat.addEventListener('click', function() {
        ...
        if (category) {
          ...
          if (category.url && isPageAvailable(category.url)) {
            ...
            // Fallback to search if category page doesn't exist
            window.SafeTradeHub.search(category.searchTerm);
          }
        }
      });
    });
  ...
  function initializeSearchFeatures() {
    // Handle search form submissions across all pages
    document.querySelectorAll('form[role="search"], .search-form, #searchForm').forEach(form => {
      form.addEventListener('submit', function(e) {
        e.preventDefault();
        const searchInput = this.querySelector('input[type="search"], .search-input, #searchInput');
        if (searchInput && searchInput.value.trim()) {
          if (window.SafeTradeHub) {
            window.SafeTradeHub.search(searchInput.value.trim());
          } else {
