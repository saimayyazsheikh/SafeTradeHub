...
async function loadProducts() {
  try {
    ...
    console.log('🧪 TESTING MODE: Loading products from localStorage');
    
    const testProducts = JSON.parse(localStorage.getItem('testProducts') || '[]');
    console.log('📦 Test products loaded:', testProducts);
    
    // Debug: Check each product's images
    testProducts.forEach((product, index) => {
      console.log(`🔍 Product ${index + 1}:`, product.name);
      console.log('   Images:', product.images);
      console.log('   Image count:', product.images ? product.images.length : 0);
      if (product.images && product.images.length > 0) {
        product.images.forEach((img, imgIndex) => {
          console.log(`   Image ${imgIndex + 1}:`, img.url, 'isMain:', img.isMain);
        });
      }
    });
    ...
    console.log('✅ Products loaded successfully from localStorage');
